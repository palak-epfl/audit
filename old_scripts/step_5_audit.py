...
# Ground truth keys — three definitions of what we are auditing against
GT_KEYS = {
    'data'       : 'dp_gap_data',        # label distribution, full local
    'model_val'  : 'dp_gap_model_val',   # model predictions, val only (unbiased)
    'model_full' : 'dp_gap_model_full',  # model predictions, full local
}
...
def run_single_audit(model, dataset, query_indices, query_gender,
    ...
    true_dp_all: dict with keys 'data', 'model_val', 'model_full'
                 — errors are computed against all three ground truths.

    Returns dict with estimated DP gap and error metrics for all three GTs.
    """
    ...
    # Compute errors against all three ground truth definitions
    for gt_name, gt_val in true_dp_all.items():
        abs_err = abs(est_dp - gt_val)
        rel_err = abs_err / gt_val if gt_val > 0 else 0.0
        result[f'true_dp_{gt_name}']  = gt_val
        result[f'abs_error_{gt_name}'] = abs_err
        result[f'rel_error_{gt_name}'] = rel_err

    # Keep primary keys pointing to model_val for backward compat with plots
    result['true_dp_gap'] = true_dp_all['model_val']
    result['abs_error']   = result['abs_error_model_val']
    result['rel_error']   = result['rel_error_model_val']
    return result
...
def audit_target_node(target_id, gpu_id, dataset, node_indices,
                      true_dp_gaps, true_dp_gaps_data, true_dp_gaps_model_val,
                      true_dp_gaps_model_full,
    ...
    try:
        ...
        true_dp_all = {
            'data'       : true_dp_gaps_data[target_id],
            'model_val'  : true_dp_gaps_model_val[target_id],
            'model_full' : true_dp_gaps_model_full[target_id],
        }
        true_dp = true_dp_gaps_model_val[target_id]  # primary for logs
        N       = len(dataset)
        ...
        for auditor_id in range(1, num_nodes + 1):
            ...
            r = run_single_audit(model, dataset, auditor_indices,
                                 auditor_gender, true_dp_all, device)
            r.update({
                'mode'       : 'full_local',
            ...
            for budget in budget_sizes:
                ...
                for rep in range(num_repeats):
                    ...
                    r = run_single_audit(model, dataset, sampled_global,
                                         sampled_gender, true_dp_all, device)
                    r.update({
                        'mode'       : 'budgeted',
                        'auditor_id' : auditor_id,
                        'target_id'  : target_id,
                ...
                # Aggregate errors for all three ground truth definitions
                for gt_name in ('data', 'model_val', 'model_full'):
                    abs_errs_gt = [r[f'abs_error_{gt_name}'] for r in repeat_results]
                    rel_errs_gt = [r[f'rel_error_{gt_name}'] for r in repeat_results]
                    mean_abs_gt, _, _ = bootstrap_ci(abs_errs_gt, seed=cfg['audit']['seed'])
                    mean_rel_gt, _, _ = bootstrap_ci(rel_errs_gt, seed=cfg['audit']['seed'])
                    budget_agg[f'true_dp_{gt_name}']       = true_dp_all[gt_name]
                    budget_agg[f'std_abs_error_{gt_name}'] = float(np.std(abs_errs_gt))
                    budget_agg[f'mean_abs_error_{gt_name}'] = mean_abs_gt
                    budget_agg[f'mean_rel_error_{gt_name}'] = mean_rel_gt

                # Primary keys → model_val for backward compat
                budget_agg['true_dp_gap']    = true_dp_all['model_val']
                budget_agg['mean_abs_error'] = budget_agg['mean_abs_error_model_val']
                budget_agg['mean_rel_error'] = budget_agg['mean_rel_error_model_val']
                budget_results.append(budget_agg)

                print(f'{tag} Budgeted    | Auditor {auditor_id} '
                      f'budget={budget} → '
                      f'mean_est_dp={mean_dp:.4f} ± {np.std(est_dps):.4f}  '
                      f'mean_abs_err={budget_agg["mean_abs_error"]:.4f}', flush=True)

        # ── Mode 3: Global audit (full dataset → this target) ─────────────────
        r_global = run_single_audit(model, dataset, global_indices,
                                    global_gender, true_dp_all, device)
        r_global.update({
            'mode'      : 'global',
            'auditor_id': 'global',
            'target_id' : target_id,
...
def main():
    ...
    # ── Load partition ─────────────────────────────────────────────────────────
    partition_fname = f'partition_alpha{ALPHA}_seed{PART_SEED}.json'
    ...
    with open(step4_path) as f:
        step4 = json.load(f)
    # All three ground truth definitions — loaded for all nodes
    true_dp_gaps_data       = {r['node_id']: r['dp_gap_data']        for r in step4['nodes']}
    true_dp_gaps_model_val  = {r['node_id']: r['dp_gap_model_val']   for r in step4['nodes']}
    true_dp_gaps_model_full = {r['node_id']: r['dp_gap_model_full']  for r in step4['nodes']}
    node_stats              = {r['node_id']: r                        for r in step4['nodes']}

    # Primary true_dp_gaps used for ranking plots — model_val (unbiased)
    true_dp_gaps = true_dp_gaps_model_val

    log.info(f"\n  True DP gaps from Step 4:")
    log.info(f"  {'Node':<8} {'Data':>10} {'Model Val':>12} {'Model Full':>12}")
    log.info(f"  " + "-" * 46)
    for node_id in sorted(true_dp_gaps_data.keys()):
        log.info(f"  Node {node_id:<4} "
                 f"{true_dp_gaps_data[node_id]:>10.4f} "
                 f"{true_dp_gaps_model_val[node_id]:>12.4f} "
                 f"{true_dp_gaps_model_full[node_id]:>12.4f}")
    ...
    log.info(f"  {'Auditor':>8} → {'Target':<6} {'Est DP':>8} "
             f"{'Err(data)':>10} {'Err(mdl_val)':>13} {'Err(mdl_full)':>14}")
    log.info("  " + "-" * 66)
    for r in sorted(full_results, key=lambda x: (x['auditor_id'], x['target_id'])):
        log.info(f"  Node {r['auditor_id']:>2}    → Node {r['target_id']:<3} "
                 f"{r['est_dp_gap']:>8.4f} "
                 f"{r['abs_error_data']:>10.4f} "
                 f"{r['abs_error_model_val']:>13.4f} "
                 f"{r['abs_error_model_full']:>14.4f}")

    for gt_name, gt_label in [('data','Data'), ('model_val','Model Val'), ('model_full','Model Full')]:
        abs_errs_gt = np.array([r[f'abs_error_{gt_name}'] for r in full_results])
        rel_errs_gt = np.array([r[f'rel_error_{gt_name}'] for r in full_results])
        log.info(f"  [{gt_label}] abs err: {abs_errs_gt.mean():.4f} ± {abs_errs_gt.std():.4f}  "
                 f"rel err: {rel_errs_gt.mean():.1%} ± {rel_errs_gt.std():.1%}")
    ...
    for r in sorted(global_results, key=lambda x: x['target_id']):
        log.info(f"  Global → Node {r['target_id']}  "
                 f"est_dp={r['est_dp_gap']:.4f}  "
                 f"err(data)={r['abs_error_data']:.4f}  "
                 f"err(mdl_val)={r['abs_error_model_val']:.4f}  "
                 f"err(mdl_full)={r['abs_error_model_full']:.4f}")
    ...
    # ── Save results JSON ──────────────────────────────────────────────────────
    def _gt_summary(results_list, gt_name):
        errs = np.array([r[f'abs_error_{gt_name}'] for r in results_list])
        rels = np.array([r[f'rel_error_{gt_name}'] for r in results_list])
        return {'mean_abs': float(errs.mean()), 'std_abs': float(errs.std()),
                'mean_rel': float(rels.mean()), 'std_rel': float(rels.std())}
    ...
    results = {
        ...
        'true_dp_gaps_model_val' : true_dp_gaps_model_val,
        'true_dp_gaps_model_full': true_dp_gaps_model_full,
        'full_results'           : full_results,
        'budget_results'         : budget_results,
        'global_results'         : global_results,
        'summary': {
            'data'       : _gt_summary(full_results, 'data'),
            'model_val'  : _gt_summary(full_results, 'model_val'),
            'model_full' : _gt_summary(full_results, 'model_full'),
        }
    }
