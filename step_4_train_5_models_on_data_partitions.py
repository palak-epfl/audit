...
Run with:
    python3 step_4_train_5_models_on_data_partitions.py --config config.yaml
...
def train_node(node_id, gpu_id, dataset, node_indices, result_queue,
    ...
    try:
        ...
        # dp_gap_data: ground-truth DP over the full node partition.
        # Uses all node indices (train + val) for the best estimate of the
        # node's local data distribution. No model inference needed.
        full_ds     = CelebADataset(dataset, node_indices, transform=eval_transform)
        p_m_true, p_f_true, dp_gap_data = compute_dp_gap(
            full_ds.smiling, full_ds.gender)

        # dp_gap_model + accuracy: val split only (model hasn't seen this data).
        val_preds, val_labels = get_predictions(model, val_loader, device)
        acc = accuracy_score(val_labels, val_preds)
        p_m_pred, p_f_pred, dp_gap_model = compute_dp_gap(val_preds, val_ds.gender)
        ...
        result = {
            ...
            # accuracy + dp_gap_model: evaluated on val split (unbiased — not seen during training)
            'accuracy'           : float(acc),
            'p_smile_pred_male'  : p_m_pred,
            'p_smile_pred_female': p_f_pred,
            'dp_gap_model'       : dp_gap_model,
            # dp_gap_data: ground truth over full node partition (train+val) for best distributional estimate
...
def main():
    ...
    log.info(f"  Partition attr : {PARTITION_ATTR}")
    ...
    partition_path  = os.path.join(PARTITION_DIR, RUN_DATE, partition_fname)
    ...
    step3_fname  = f'step3_{ALPHA}_{PARTITION_ATTR}_partition_stats.json'
    results_path = os.path.join(RESULTS_DIR, step3_fname)
    ...
    log.info(f"  Partition: α={ALPHA}  attr={PARTITION_ATTR}  seed={PART_SEED}")
    log.info(f"{'─'*70}\n")
